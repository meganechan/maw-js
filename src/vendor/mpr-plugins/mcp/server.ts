  ...
  server.registerTool(
    "maw_task",
    {
      ...
      inputSchema: {
        ...
        state: z.enum(["backlog", "todo", "ready", "approve", "need-answer", "wait-for-deploy", "external-wait", "review"]).optional().describe("action-specific: add only backlog|todo|approve (approve requires reason); move only backlog|todo|ready|approve|need-answer|wait-for-deploy (approve/need-answer require reason); reopen backlog|todo|ready|review; external-wait uses trigger + dedicated action, not state"),
        assignee: z.string().optional().describe("add: assignee oracle"),
        needs: z.array(z.string()).optional().describe("add: dependency card ids (preferred — kobo-640)"),
        parent: z.array(z.string()).optional().describe("add: dependency card ids, deprecated alias for `needs` (kobo-640) · dep: the ONE parent id to link/unlink"),
        ...
        full: z.boolean().optional().describe("ls (kobo-368): full per-card board render; default is a compact lane-count summary"),
        ...
        reviewerCell: z.string().optional().describe("add/review: durable Cell v2 reviewer cell id (--reviewer-cell) for cross-cell routing"),
        ...
        evidenceScope: z.enum(["producer", "independent", "epic"]).optional().describe("evidence: structured Cell v2 evidence scope"),
        changed: z.string().optional().describe("evidence: what changed"),
        verified: z.string().optional().describe("evidence: how it was verified"),
        ref: z.string().optional().describe("evidence: optional branch/ref"),
        sha: z.string().optional().describe("evidence: optional commit/content sha"),
        locus: z.string().optional().describe("evidence: where the proof lives"),
        limitations: z.string().optional().describe("evidence: what this proof does not cover"),
        trigger: z.string().optional().describe("external-wait: named signal that wakes the card"),
