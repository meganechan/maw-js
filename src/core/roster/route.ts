 ...
 * (Rule 6), same surface as /api/state.
 *
...
export function handleRosterRequest(request: Request): Response {
  ...
  // `held` (kobo-105): { oracle → open worklog claims } so the Presence tab can
  // flag an idle-looking oracle that is actually holding work. The card half of
  // this projection (in-progress board cards) and the `pending` field (kobo-445,
  // every non-terminal card) went with the task subsystem — the claim registry is
  // the ownership signal maw still owns.
