...
4. **coord spawn workers** — ตาม crew §1 เป๊ะ (contract-to-file + `--settings .claude/crew-worker-settings.json` + `CREW_ROLE=worker-N CREW_COORD_PANE=$COORD`) → Stop hook ยิง idle เข้า **coord** (ไม่ใช่ lead)
