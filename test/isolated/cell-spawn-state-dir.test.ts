...
let commands: string[] = [];
/** `id|||@role|||window|||@oracle_pane|||path` — the head starts alone, carrying
 *  the `{oracle}:head` stamp `wake` put on it. Cell reads that; it never writes
 *  it, and refuses when it is missing (kobo-822). */
let paneRows: string[] = [];
...
beforeEach(() => {
  commands = [];
  paneRows = [["%head", "", "patchwork-oracle", "patchwork:head", "/tmp"].join("|||")];
