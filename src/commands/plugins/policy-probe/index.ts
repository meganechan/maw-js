 ...
 * an exit code:
 *   0  inject reached and its brain clone is not behind
...
import { probePolicyInject, policyPort, policyHookPath, type PolicyProbeResult } from "../../../core/policy/probe";
  ...
  if (args.some(a => a === "-h" || a === "--help")) {
    return {
      ...
      output: [
        ...
        "",
        "  exit 0 = reached · 1 = did not reach · 2 = reached but the brain clone is behind",
      ].join("\n"),
    };
  }
...
/** Probe result → CLI result. `stage` set ⇒ never arrived (1); else stale brain (2). */
export function toResult(r: PolicyProbeResult): InvokeResult {
  const output = formatProbe(r);
  return r.ok ? { ok: true, output } : { ok: false, exitCode: r.stage ? 1 : 2, output };
}
