...
export function buildInjectSlice(oracle: string, opts: SliceOpts = {}): string {
  ...
  // Exclude 'idle' (kobo-109) + 'away' (mawjs-3) + 'back' (kobo-120): per-pane state signals,
  // not activity — they'd otherwise dominate the small inject window and starve real activity out.
  const recent = readWorklog(company, { limit: opts.events ?? DEFAULT_EVENTS, excludeKinds: ["idle", "away", "back"] });
