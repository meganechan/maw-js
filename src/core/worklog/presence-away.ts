...
export function companyOfOracleLight(oracle: string): string | null {
  ...
  for (const f of files) {
    try {
      ...
      // kobo-737: `teams` is the only schema key (the legacy `departments`
      // fallback is gone). This reader bypasses loadCompany's normalization on
      // purpose — barrel-free twin — so a legacy file just resolves to no
      // company here; loadCompany is the one that fails loudly about it.
