...
mock.module("maw-js/sdk", () => ({
  ...
  findWindow: (_s: unknown, oracle: string) => {
    const map: Record<string, string> = { patchwork: "sessA:patchwork", stitch: "sessB:stitch" };
...
beforeEach(() => {
  ...
  nextPane = 0;
  sessions = {
    sessA: [{ id: "%headA", window: "patchwork", identity: "" }],
    sessB: [{ id: "%headB", window: "stitch", identity: "" }],
  };
