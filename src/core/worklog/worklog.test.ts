...
describe("hook scripts stay in sync with embedded base64", () => {
  it("decoded base64 matches scripts/hooks/*.sh", async () => {
    ...
    for (const f of ["worklog-tool.sh", "worklog-convo.sh", "worklog-orient.sh", "company-policy.sh", "toilet-away.sh", "seat-back.sh", "maw-mcp-nudge.sh", "agent-offload-nudge.sh", "maw-statusline.sh"]) {
