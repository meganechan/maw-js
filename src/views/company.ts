...
/**
 * company-ui (read-only) — activity + coordination panels for one company.
 *
 ...
 *   - presence      who's around             from GET /api/roster + /api/presence
 *
 * The kanban board panel (and its card-detail modal, its /api/tasks poll and
 * every /api/tasks/* write) retired with the task subsystem — the board lives in
 * kobo taskd now. Generic via ?company= → same renderer serves kobo, pgw, …
 */
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 880px) { body { padding:12px; } .timeline, .md { max-height:none; } }
...
<body>
  ...
  <nav class="tabs" role="tablist" aria-label="views">
    <button type="button" class="tab active" data-tab="state" role="tab" aria-selected="true">State<span class="tab-count" id="tab-count-state"></span></button>
...
function renderPresence(entries, roster, presence, held) {
  ...
  for (const item of rows) {
    ...
    // see WHAT is stuck (open claim ids), not just that something is.
    if (idleWithWork) {
...
async function load() {
  ...
  if (!company) { statusEl.textContent = 'specify ?company= (e.g. /company?company=pgw)'; lastEntries = []; lastRoster = []; lastHeld = {}; lastPresence = []; lastState = null; renderTimeline([]); renderState(null); updateTabCounts(); if (activeTab === 'presence') renderPresence([], [], [], {}); return; }
  ...
  try {
    ...
    const [feedRes, stateRes, rosterRes, presenceRes] = await Promise.all([
    ...
    statusEl.textContent = wlN + ' worklog entr' + (wlN === 1 ? 'y' : 'ies') + (stateRes && stateRes.exists ? ' · state.md' : '');
  } catch (err) {
...
// A browser holding the retired 'kanban' tab in localStorage falls through the
// querySelector guard to 'state' — never to a hidden-everything blank page.
(function () { let t = 'state'; try { t = localStorage.getItem('maw-company-tab') || 'state'; } catch (e) { /* private mode */ } if (!document.querySelector('.tab[data-tab="' + t + '"]')) t = 'state'; showTab(t); })();

// kobo-37: auto-refresh — poll every 5s so the panels track changes without F5.
...
load(); // paint before the first 5s tick
checkVersion();
startPoll();
</script>
</body>
