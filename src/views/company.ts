...
export function stateBadge(task) {
  ...
  const LABEL = {
    ...
    review: 'Review', 'need-answer': 'Need answer', approve: 'Approve', 'wait-for-deploy': 'Wait for deploy', 'external-wait': 'External wait', blocked: 'Blocked', done: 'Done', rejected: 'Rejected',
  };
...
export const COLLAPSIBLE_COLS = ["backlog", "rejected", "wait-for-deploy", "external-wait"];
...
export const DEFAULT_OPEN_COLS = ["wait-for-deploy", "external-wait"];
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .col-in-progress h2 { color:var(--accent); } .col-review h2 { color:var(--epic); } .col-need-answer h2 { color:var(--warn); } .col-approve h2 { color:var(--link); } .col-wait-for-deploy h2 { color:var(--link); } /* kobo-273 — merged≠live park */ .col-external-wait h2 { color:var(--warn); } /* Cell v2 — non-WIP wait on named external trigger */ .col-done h2 { color:var(--ok); }
    ...
    .task.st-external-wait { border-left-color:var(--warn); } /* Cell v2 — waiting on external signal */
    ...
    .pill.state.st-external-wait { color:var(--warn); border-color:var(--bd-warn); }
    ...
    <section class="tabpanel" data-tab="kanban" role="tabpanel">
      <div class="card">
        ...
        <div class="board">
          ...
          <div class="col col-external-wait"><h2><button class="col-chevron" type="button" data-col="external-wait" aria-label="toggle external-wait">▸</button><span>⏳&nbsp;External&nbsp;wait</span><span class="count" id="c-external-wait">0</span></h2><div id="external-wait"></div></div>
...
const COLS = ['backlog', 'todo', 'ready', 'in-progress', 'review', 'need-answer', 'approve', 'wait-for-deploy', 'external-wait', 'blocked', 'done', 'rejected']; // board columns = flow + need-answer (kobo-218, off-flow Tony queue) + wait-for-deploy (kobo-273, merged≠live park) + external-wait (Cell v2, non-WIP named trigger) + Blocked (kobo-199, off-flow) + Rejected terminal lane (kobo-101)
...
function renderBoard(tasks) {
  ...
  const counts = { backlog: 0, todo: 0, ready: 0, 'in-progress': 0, review: 0, 'need-answer': 0, approve: 0, 'wait-for-deploy': 0, 'external-wait': 0, done: 0, rejected: 0, blocked: 0 };
